...
export default function AdminPerformancePage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-xl font-semibold text-slate-800">Student performance</h2>
        <p className="text-sm text-slate-500">Average scores across submitted tests. Click a student for details.</p>
      </div>
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.studentId} className="border-b border-slate-50 last:border-0">
                    ...
                    <td className="px-5 py-3 text-slate-600">{r.totalAttempts}</td>
                    <td className="px-5 py-3 text-slate-600">{r.totalAttempts > 0 ? `${r.averagePercent}%` : '-'}</td>
